...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float4 _Color_3_var = UNITY_ACCESS_INSTANCED_PROP( Props, _Color_3 );
                float4 _Albedo_var = tex2D(_Albedo,TRANSFORM_TEX(i.uv0, _Albedo));
                float3 diffColor = lerp(lerp(_Color_var.rgb,_Color_2_var.rgb,pow(Object_TPM( _Tile , i.posWorld.rgb , objPos.rgb , i.normalDir , _Detail_var , _Scale_var ),_Blend_var)),_Color_3_var.rgb,_Albedo_var.rgb);
                float specularMonochrome;
                float3 specColor;
                float _Metallic_var = UNITY_ACCESS_INSTANCED_PROP( Props, _Metallic );
